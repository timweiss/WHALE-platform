...
public class BootUpReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals("android.intent.action.BOOT_COMPLETED")) {
            ...
            if (!samplingManager.isRunning(context.getApplicationContext())) {
                samplingManager.startSampling(context.getApplicationContext());
            }
        } else {
            Log.e("BootUpReceiver", "invalid intent action: " + intent.getAction());
        }
    }
}